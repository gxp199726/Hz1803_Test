...
object DWD_DM {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName(Constan.SPARK_APP_NAME_USER).master(Constan.SPARK_LOCAL).config("spark.debug.maxToStringFields", "100").enableHiveSupport().getOrCreate()
